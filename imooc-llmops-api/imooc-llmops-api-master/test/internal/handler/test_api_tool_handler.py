...
class TestApiToolHandler:
    ...
    @pytest.mark.parametrize("query", [
        ...
        {"current_page": 2},
        {"search_word": "高德"},
        {"search_word": "LLMOps-Platform 工具包"},
    ])
    def test_get_api_tool_providers_with_page(self, query, client):
        ...
        elif query.get("search_word") == "LLMOps-Platform 工具包":
            assert len(resp.json.get("data").get("list")) == 0
        else:
            assert resp.json.get("code") == HttpCode.SUCCESS
    ...
    def test_create_api_tool_provider(self, client, db):
        ...
        api_tool_provider = db.session.query(ApiToolProvider).filter_by(name="LLMOps-Platform 学习工具包").one_or_none()
