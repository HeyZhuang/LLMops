...
class FaissService:
    ...
    def __init__(self, embeddings_service: EmbeddingsService):
        ...
        # 3.初始化faiss向量数据库
        #    如果当前环境未安装 faiss，则优雅降级为 None，避免阻塞整个后端启动
        try:
            ...
            # 未安装 faiss 时，仅关闭基于本地 Faiss 的知识库检索能力
            self.faiss = None
    ...
    def convert_faiss_to_tool(self) -> BaseTool:
        ...
        if self.faiss is None:
            raise FailException(
                "当前环境未安装 faiss，知识库检索工具暂不可用，请安装 faiss-cpu 或 faiss-gpu 后重试"
            )
